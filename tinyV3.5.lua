...
function Tiny.Skycombo()
    ...
    local avalanche = NPC.GetAbilityByIndex(myHero, 1)
    local toss = NPC.GetAbilityByIndex(myHero, 0)
    ...
    local mousePos = Input.GetWorldCursorPos()
        ...
        if Menu.IsKeyDown(Tiny.optionComOrder) then
            ordernow = ordernow +1
            if ordernow == 8 then ordernow = 1 end
            customCastNow = Menu.GetValue(Tiny.orders[ordernow])
        end
        lastcasttime = os.clock()

    -- 2. Blink
    if blink and Menu.IsEnabled(Tiny.optionBlink) and Ability.IsCastable(blink, myMana) and (not Menu.IsKeyDown(Tiny.optionComOrder) or customCastNow == 2) then Ability.CastPosition(blink, heroPos); return end  
    ...
    --6. Avalanche
    if Menu.IsEnabled(Tiny.optionEnable) and avalanche and Ability.IsCastable(avalanche, myMana) and (not Menu.IsKeyDown(Tiny.optionComOrder) or customCastNow == 6) then Ability.CastTarget(avalanche, hero); return end

    --7. Toss
    if Menu.IsEnabled(Tiny.optionEnable) and toss and Ability.IsCastable(toss, myMana) and (not Menu.IsKeyDown(Tiny.optionComOrder) or customCastNow == 7) then Ability.CastPosition(toss, heroPos); return end
